...
print("Tailscale status:", result.data.stdout)

# Verify connection
status = sandbox.exec("sudo tailscale status")
print(status.data.stdout)
```
...
print("Internal API response:", result.data.stdout)
...
print("Query result:", query.data.stdout)
```
...
print("Tailscale status:", json.loads(status.data.stdout))

# Get current IP address
ip = sandbox.exec("sudo tailscale ip -4")
print("Tailscale IP:", ip.data.stdout.strip())
```
