...
print("VNC sessions:", result.data.stdout)
```
...
vr = VoidRun(api_key=os.environ.get("VR_API_KEY"))
sandbox = vr.get_sandbox("vnc-sandbox")
# Install browser and automation tools
...
<Warning>
**SSH and VNC connections drop when the sandbox is snapshotted.** Idle sandboxes are auto-slept: their VM process exits and existing TCP sessions are torn down. Keep the sandbox alive with periodic activity if you rely on long-lived interactive sessions.
</Warning>
...
// Option 1: Wake sandbox if it snapshotted
await sandbox.wake();

// Option 2: Keep-alive with periodic activity
// Tune the interval to sit well inside your deployment's idle threshold.
setInterval(async () => {
  await sandbox.exec({ command: 'echo "keepalive" > /dev/null' });
}, 30 * 1000);

// Option 3: Run a long-running process (Commands API returns a PID)
await sandbox.commands.run('while true; do sleep 60; done', undefined, undefined, 0);
```
...
```bash CLI
# Wake sandbox if snapshotted
voidrun wake <sandbox-id>
...
if (sandbox.status === 'snapshotted') {
  console.log('Sandbox is snapshotted, waking...');
  await sandbox.wake();
  console.log('Sandbox running, ready for SSH/VNC');
}
...
sandbox = vr.get_sandbox("sandbox-id")
if sandbox.status == "snapshotted":
    print("Sandbox is snapshotted, waking...")
    sandbox.wake()
    print("Sandbox running, ready for SSH/VNC")
