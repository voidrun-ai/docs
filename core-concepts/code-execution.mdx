...
<Note>
For a **single reference** of every shell and process path (sync **`exec`**, streaming, **`commands.*`**, **`runCode`**, PTY, **`session-exec`**), see [Shell execution](/core-concepts/shell-execution).
</Note>
...
print("stdout:", result.data.stdout)
print("stderr:", result.data.stderr)
...
# Access output fields
print("Standard Output:", result.data.stdout)
print("Standard Error:", result.data.stderr)
print("Exit Code:", result.data.exit_code)
...
# Code interpreter response structure (CodeExecutionResult)
print("Success:", result.success)        # boolean
print("Stdout:", result.stdout)          # raw output
print("Stderr:", result.stderr)          # errors
print("Exit Code:", result.exit_code)    # process exit code
print("Results:", result.results)        # parsed last line
print("Logs:", result.logs)              # {"stdout": [], "stderr": []}
```
...
print(result.data.stdout)  # /app/src
```
...
// Start a background process (command, env?, cwd?, timeout?)
const runResult = await sandbox.commands.run('npm run dev', undefined, '/app');
...
run_result = sandbox.commands.run("npm run dev", None, "/app")
