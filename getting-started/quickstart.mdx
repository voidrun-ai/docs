...
Create an isolated MicroVM environment. The examples use the **`code`** system image (Alpine-based, with Python and Node: see [System images](/environments/custom-images#system-images)). You can use **`max`** (Debian) or **`docker`** (Docker-in-Docker) instead when your workload requires them.
